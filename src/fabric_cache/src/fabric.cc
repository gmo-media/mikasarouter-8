...
bool Fabric::connect() noexcept {
  ...
  mysql_options(fabric_connection_, MYSQL_OPT_READ_TIMEOUT, &connection_timeout_);
  mysql_options(fabric_connection_, MYSQL_OPT_WRITE_TIMEOUT, &connection_timeout_);
  ...
  if (mysql_real_connect(fabric_connection_, host.c_str(), user_.c_str(),
    ...
    if (reconnect_tries_++ % 5 == 0) {
      log_error("Failed connecting with Fabric: %s (tried %d time%s)",
                mysql_error(fabric_connection_), (int) reconnect_tries_,
